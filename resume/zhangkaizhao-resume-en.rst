...
* Back-end development: Django, Flask, Tornado, SQL, Redis, etc.
* Development environment: GNU/Linux and FreeBSD, especially Debian, Ubuntu, Fedora, Arch Linux, etc.
* Toolkits: Git, Vim, UNIX utilities, etc.
* Others: Agile software development(e.g. Scrum), Continuous Integration(aka CI), etc.
...
*Note: Intel Security was departed from Intel since April 2017 as new McAfee. The contract was ended on April 3, 2017. Then I kept in the new McAfee until May 26, 2017.*
...
* English: passed *College English Test Band 4* (*CET-4*) in March 2004
...
* Improved features including withdrawal, order, payment, ticketing, back-end management functions, etc.
* Improved structure and quality of source code.
...
* Router Hub server of WIT-FII project.
* Features:

  - API service for management of accounts and routers, etc.
  - Gateway service for routers.
  - Online store, etc.
...
* Gateway service works well for routers.
...
* Features of web server:
  ...
  - Website for online browse and file operations.
  - API service for mobile clients.

* The DDNS server provides Dynamic DNS service for every machine, which can connect to internet and has Syncbox servers running on it.

Working on:

* Preview image, thumbnail and metadata generation for files including image, PDF, Office, audio, video, plain text, etc files.
* Recent files sync to Dropbox, Google Drive, SkyDrive, Jingshan Kuaipan and Kupan.
* Development of API service.

Achievement:

* Added preview image, thumbnail and metadata API for website, iOS and Android client.
